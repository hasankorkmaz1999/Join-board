...
function openMobileMenu(key) {
  ...
  if (menu.classList.contains("show")) {
    ...
    const imgElement = document.querySelector(
      `img[onclick="openMobileMenu('${key}')"]`
    );
...
document.addEventListener("click", function (event) {
  ...
  if (
    !targetElement.classList.contains("responsive-dots") &&
    !targetElement.closest(".miniMenu")
  ) {
...
function renderDivTodo(task, key) {
  let categoryClass =
    task.category === "Technical Task" ? "technical-green" : "user-story-blue";
